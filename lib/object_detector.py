    ...
    def forward(self, images_list, annotations_list, indices, sampled_frames):
        ...
        for idx, (images, annotations) in enumerate(zip(images_list, annotations_list)):
            for frame_idx, (image, anns) in enumerate(zip(images, annotations)):
                ...
                for ann in anns:
                    print({
                        "bbox": ann['bbox'].cpu().tolist(),
                        "class": self.object_classes[ann['class']]
                    })
                ...
                if self.mode == 'predcls':
                    ...
                    for human_idx in human_indices_in_frame:
                        for obj_idx in object_indices_in_frame:
                            ...
                            IM_IDX.append(image_index)
                # 打印当前帧的检测结果
                print("Detection Results (Mode:", self.mode, "):")
                print("Boxes:", [box.cpu().tolist() for box in FINAL_BBOXES])
                print("Labels:", [self.object_classes[label] for label in FINAL_LABELS])
                print("Scores:", [score.item() for score in FINAL_SCORES])
        # 将列表转换为张量
        if FINAL_BBOXES:
